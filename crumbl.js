...
(function () {
	...
	function manageEvent(event, handler, remove, nodes) {
		nodeLoop(function (elm) {
			...
			if(remove){
				...
				// elm[event] =  function () { return handler.apply(elm, arguments); };
				elm[event] = handler;
			}
		}, nodes);
	}
	...
	function manageClass(classes, action, nodes) {
		var values = [], classarray, classname, search, has, i, get;
	...
	function crumbl(selector){
        ...
        var r = [];
          ...
          var f = qwery(s, n),
            ...
            for(i=0; i<l; i++){
              if(typeof f[i] !== 'undefined'){ r.push(f[i]); }
            }
        }, nodes);
